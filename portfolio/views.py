...
def portfolio_detail_view(request, pk):
    ...
    for tx in portfolio.transactions.order_by('-transaction_date'):
        ...
        if tx.transaction_type == 'BUY':
            ...
            # FIFO cost basis — matches TaxReportService exactly
            fifo_cost = float(FXLotService._fifo_cost_basis_stock_currency(
                portfolio, tx.symbol, tx.quantity, tx_date
            ))
            fifo_avg = fifo_cost / qty if qty else None
            pnl = round(price * qty - commission - fifo_cost, 2)
            chips = [f"Qty: {_fmt_qty(qty)}", f"Price: {_fmt_cur(tx_cur, price)}"]
            if fifo_avg is not None:
                chips.append(f"Avg Cost: {_fmt_cur(tx_cur, fifo_avg)}")
            if commission:
                chips.append(f"Commission: {_fmt_cur(tx_cur, commission)}")
